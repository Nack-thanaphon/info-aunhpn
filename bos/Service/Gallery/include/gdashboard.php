        ...
        <a href="<?php echo base_url ?>?page=albums/images&id=<?php echo $row['id'] ?>" class="album-item">
            <div class='album-view'>
                <?php
					foreach ($img as $path) :
					?>
                <img src="<?php echo validate_image($path) ?>" class="img-thumbnail img-fluid album-banner" alt="img"
                    loading="lazy">
                <?php endforeach; ?>
                <?php if (count($img) == 0) : ?>
                <img src="<?php echo validate_image('') ?>" class="img-thumbnail img-fluid album-banner" alt="img"
                    loading="lazy">
                <?php endif; ?>
            </div>
            <div class="w-100 d-flex justify-content-between">
                ...
                <div class="dropleft">
                    <a href="#" id="menus_<?php echo $row['id'] ?>" data-toggle="dropdown" aria-haspopup="true"
                        aria-expanded="false" class="text-dark"><i class="fa fa-ellipsis-v"></i> </a>
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                        <a class="dropdown-item edit_album" data-id="<?php echo $row['id'] ?>"
                            href="javascript:void(0)"><i class="fa fa-edit text-primary"></i> Rename</a>
                        <div class="dropdown-divider"></div>
                        <a class="dropdown-item delete_album" data-id="<?php echo $row['id'] ?>"
                            href="javascript:void(0)"><i class="fa fa-trash text-danger"></i> Remove</a>
                    </div>
                </div>
            </div>
        </a>
